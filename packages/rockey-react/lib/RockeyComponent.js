...
export default class RockeyComponent extends React.Component {
  ...
  componentWillMount() {
    ...
    this.staticClassName = Object.values(classList).join(' ');
  ...
  updateCSSVariables(props) {
    ...
    Object.keys(CSSVariables).forEach(v => {
      // TODO: rm propterty
      dom.style.setProperty(`--${v}`, CSSVariables[v]);
    });
  }
  ...
  render() {
    ...
    // console.log(dynamicClassList);
    const className = classnames(
      this.staticClassName,
      // dynamicClassList[selector],
      Object.values(dynamicClassList).join(' '),
